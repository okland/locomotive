...
exports = module.exports = function server(dir, address, port, env, options) {
  ...
  locomotive.boot(dir, env, function(err, server) {
    ...
    server.listen(port, address, function() {
        ...
        // add socket io redis based connections
        var io = require('socket.io'),
            redis = require('redis');
        var theSocket = io.listen(this);
        var RedisStore = require('socket.io/lib/stores/redis');
        var client = redis.createClient();

        // use redis as store (default is memory)
        theSocket.set('store', new RedisStore({
            redisClient : client
        }));

        var datObject = {};
        theSocket.sockets.on('connection', function (socket) {

            // simple event: receive a msg and broadcast it to all clients
            socket.on('msg',function (data) {
                var count = datObject[data.code] = datObject[data.code] ? datObject[data.code] : 0;
                console.log('new message to broadcast',data);
                socket.emit('msg',count);
            });
            socket.on('plus',function(data){
                datObject[data.code] = datObject[data.code] ? datObject[data.code] + 1: 1;
                console.log('new message to broadcast',data);
                socket.emit('msg',datObject[data.code]);
            });
        });
